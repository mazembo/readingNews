...
# Write a generic downloadImage function. Receiving image url and page url. If the image url is relative, the function
# should make an absolute url.

def downloadImage(images_url, url):
...
def cleanUp(title, body, short_message, tweet_message):
    title = title.replace(":", "--")
    body =  body.replace(":", "--")
...
def getFormated(title, body, short_message, tweet_message, date_accessed, date_published, url, image_filename):
...
def processLinks(textfile, today_date):
...
def makeSoup(url):
    ...
    tweet_message = title
    return html, soup, date_accessed, title, tweet_message
